...
public class SettingsActivity extends AppCompatPreferenceActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    static public float nmsThreshold = 0.0f;
    static public int[] inputFpnStride = new int[]{};
    ...
    EditTextPreference etInputFpnStride = null;
    EditTextPreference etNmsThreshold = null;
    ...
    List<String> preInstalledPicoDetConfig = null;
    List<String> preInstalledInputFpnStrides = null;
    List<String> preInstalledNmsThresholds = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        preInstalledInputFpnStrides = new ArrayList<String>();
        preInstalledNmsThresholds = new ArrayList<String>();
        preInstalledModelDirs.add(getString(R.string.MODEL_DIR_DEFAULT));
        preInstalledLabelPaths.add(getString(R.string.PICODET_CONFIG_DEFAULT));
        preInstalledPicoDetConfig.add(getString(R.string.LABEL_PATH_DEFAULT));
        ...
        preInstalledInputFpnStrides.add(getString(R.string.INPUT_FPN_STRIDE_DEFAULT));
        preInstalledNmsThresholds.add(getString(R.string.NMS_THRESHOLD_DEFAULT));
        ...
        etInputFpnStride = (EditTextPreference) findPreference(getString(R.string.INPUT_FPN_STRIDE_KEY));
        etNmsThreshold = (EditTextPreference) findPreference(getString(R.string.NMS_THRESHOLD_KEY));
    }
    ...
    private void reloadSettingsAndUpdateUI() {
        ...
        if (selected_model_idx >= 0 && selected_model_idx < preInstalledModelDirs.size() && selected_model_idx != selectedModelIdx) {
            ...
            editor.putString(getString(R.string.PICODET_CONFIG), preInstalledPicoDetConfig.get(selected_model_idx));
            editor.putString(getString(R.string.INPUT_FPN_STRIDE_KEY), preInstalledInputFpnStrides.get(selected_model_idx));
            editor.putString(getString(R.string.NMS_THRESHOLD_KEY), preInstalledNmsThresholds.get(selected_model_idx));
            editor.commit();
        ...
        String input_fpn_stride = sharedPreferences.getString(getString(R.string.INPUT_FPN_STRIDE_KEY),
                getString(R.string.INPUT_FPN_STRIDE_DEFAULT));
        String nms_threshold = sharedPreferences.getString(getString(R.string.NMS_THRESHOLD_KEY),
                getString(R.string.NMS_THRESHOLD_DEFAULT));
        ...
        etInputFpnStride.setSummary(input_fpn_stride);
        etInputFpnStride.setText(input_fpn_stride);
        etNmsThreshold.setSummary(nms_threshold);
        etNmsThreshold.setText(nms_threshold);
    }
    ...
    static boolean checkAndUpdateSettings(Context ctx) {
        ...
        String config_path = sharedPreferences.getString(ctx.getString(R.string.PICODET_CONFIG),
                ctx.getString(R.string.PICODET_CONFIG_DEFAULT));
        settingsChanged |= !configPath.equalsIgnoreCase(config_path);
        ...
        String nms_threshold = sharedPreferences.getString(ctx.getString(R.string.NMS_THRESHOLD_KEY),
                ctx.getString(R.string.NMS_THRESHOLD_DEFAULT));
        settingsChanged |= nmsThreshold != Float.parseFloat(nms_threshold);
        nmsThreshold = Float.parseFloat(nms_threshold);

        String input_fpn_stride = sharedPreferences.getString(ctx.getString(R.string.INPUT_FPN_STRIDE_KEY),
                ctx.getString(R.string.INPUT_FPN_STRIDE_DEFAULT));
        int[] int_array_data = Utils.parseIntsFromString(input_fpn_stride, ",");
        settingsChanged |= int_array_data.length != inputFpnStride.length;
        if (!settingsChanged) {
            for (int i = 0; i < int_array_data.length; i++) {
                settingsChanged |= int_array_data[i] != inputFpnStride[i];
            }
        }
        inputFpnStride = int_array_data;
